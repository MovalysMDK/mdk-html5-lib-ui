...
angular.module('mfui').factory('MFRootScopeBuilder', ['MFException', '$injector', '$state', '$stateParams', '$q', 'MFSyncPromiseProvider',
    'MFViewScopeBuilder', 'MFGenericLoadDataAction', 'MFActionLauncher', 'MFUpdateVMWithDataLoaderAction', 'MFUtils', 'MFFormModals', 'growl', 'MFConfigurationService', 'snapRemote', 'MFDeviceInfo', '$translate', 'MFCordova', '$anchorScroll', '$location', '$rootScope', '$ionicPlatform', 'IONIC_BACK_PRIORITY',
    function (MFException, $injector, $state, $stateParams, $q, $qSync, MFViewScopeBuilder, MFGenericLoadDataAction, MFActionLauncher, MFUpdateVMWithDataLoaderAction, MFUtils, MFFormModals, growl, MFConfigurationService, snapRemote, MFDeviceInfo, $translate, MFCordova, $anchorScroll, $location, $rootScope, $ionicPlatform, IONIC_BACK_PRIORITY) {
        ...
        MFRootScopeActions.prototype.setScreenScope = function ($screenScope) {
            console.assert(this.$scope.rootActions.$scope.$screenScope === null, 'You have set two screen scopes or used $state.go instead of rootActions.go');
        ...
        MFRootScopeActions.prototype.showErrorNotification = function (msg, param) {
            ...
            console.error($translate.instant(msg));
                    ...
                    if (self.isDirty()) {
                        ...
                        if (forgottenModificationsModal !== undefined) {
                            forgottenModificationsModal.result.then(function (save) {
                                ...
                                if (save) {
                                    self.save(self.$scope.$screenScope.viewConfig.screenConfig.silentSaveOnExit).then(function () {
        ...
        //  Form actions
        MFRootScopeActions.prototype.save = function (silent) {
            ...
            for (var i = 0, ii = formScopes.length; i < ii; ++i) {
                if (formScopes[i].viewConfig.saveAction) {
                    actions.push(formScopes[i].actions.save(silent));
                }
            }
            $q.all(actions).then(function () {
                if (angular.isUndefinedOrNull(self.$scope.$workspaceScope)) {
                    console.log('Save success');
                    if (!angular.isUndefinedOrNull(self.$scope.$screenScope.viewConfig.screenConfig.saveSuccessCallback)) {
                        self.$scope.$screenScope.viewConfig.screenConfig.saveSuccessCallback.call(self);
                    }
                    deferred.resolve();
                    //self.exitSaveMode();
                } else {
                    self.$scope.$workspaceScope.actions.save(silent).then(function () {
                        ...
                        if (!angular.isUndefinedOrNull(self.$scope.$screenScope.viewConfig.screenConfig.saveFailureCallback)) {
                            self.$scope.$screenScope.viewConfig.screenConfig.saveFailureCallback.call(self);
                        }
                        deferred.reject();
                    });
                }
        ...
        MFRootScopeActions.prototype.navigateNew = function () {
            ...
            for (var listScopeKey = 0; listScopeKey < this.$scope.$listScopes.length; ++listScopeKey) {
                console.assert(angular.isFunction(this.$scope.$listScopes[listScopeKey].actions.rootActions.clearItemSelection), 'the function "clearItemSelection should be added to the panel scope by the directive MfList');
                this.$scope.$listScopes[listScopeKey].actions.rootActions.clearItemSelection();
            }
        ...
        function lockScreenSize() {
            if (device.platform === 'Android') {
                var wrapper = document.getElementById('mdk-container-wrapper');
                var heightLandscape;
                var heightPortrait;
            ...
            function doOnOrientationChange() {
                ...
                setTimeout(function () {
                    switch (window.orientation) {
                        ...
                        case 90:
                            if (!heightLandscape) {
                                heightLandscape = document.documentElement.clientHeight;
                            }
                            wrapper.style['min-height'] = heightLandscape - 1 + 'px';
                            break;
                        default:
                            if (!heightPortrait) {
                                heightPortrait = document.documentElement.clientHeight;
                            }
                            wrapper.style['min-height'] = heightPortrait - 1 + 'px';
                            break;
                    }
                }, 50);
            }
