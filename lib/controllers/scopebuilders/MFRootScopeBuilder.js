...
//  MESSAGES & POPUPS
        ...
        MFRootScopeActions.prototype.navigateNew = function () {
            ...
            for (var listScopeKey = 0; listScopeKey < this.$scope.$listScopes.length; ++listScopeKey) {
                console.assert(angular.isFunction(this.$scope.$listScopes[listScopeKey].clearItemSelection) ,'the function "clearItemSelection should be added to the panel scope by the directive MfList');
                this.$scope.$listScopes[listScopeKey].clearItemSelection();
            }
