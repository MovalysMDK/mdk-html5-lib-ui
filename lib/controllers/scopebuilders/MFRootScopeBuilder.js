...
angular.module('mfui').factory('MFRootScopeBuilder', ['MFException', '$injector', '$state', '$stateParams', '$q',
    'MFViewScopeBuilder', 'MFGenericLoadDataAction', 'MFActionLauncher', 'MFUpdateVMWithDataLoaderAction', 'MFUtils', 'MFFormModals','growl','MFConfigurationService','snapRemote','MFCordova','$translate',
    function(MFException, $injector, $state, $stateParams, $q, MFViewScopeBuilder, MFGenericLoadDataAction, MFActionLauncher, MFUpdateVMWithDataLoaderAction, MFUtils, MFFormModals,growl,MFConfigurationService,snapRemote, MFCordova,$translate) {
        ...
        MFRootScopeActions.prototype.registerFormScope = function ($formScope) {
            ...
            for(;i<this.$scope.$formScopes.length;i++){
        ...
        MFRootScopeActions.prototype.unregisterFormScope = function ($formScope) {
            var index;
            if ((index = this.$scope.$formScopes.indexOf($formScope)) > -1) {
        ...
        MFRootScopeActions.prototype.registerListScope = function ($listScope) {
            ...
            for(;i<this.$scope.$listScopes.length;i++){
...
//  MESSAGES & POPUPS
        ...
        MFRootScopeActions.prototype.showInfoNotification = function(msg) {
            ...
            this.showNotification({message: msg,level:'INFO'});
        };

        MFRootScopeActions.prototype.showWarningNotification = function(msg) {
            console.warn(msg);

            this.showNotification({message: msg,level:'WARNING'});
        };
            ...
            function available(){
                if(navigator.notification && navigator.notification.alert) {
                    MFRootScopeActions.prototype.showNotification = function (m, interpolateParams, closingTimeout) {
                        ...
                        var message = getNotificationMessage(m);//TODO apply translation
                        if(m.messageParam){
        ...
        MFRootScopeActions.prototype.goReload = function() {
            ...
            if(hasChanged){
                ...
                if (angular.isUndefinedOrNull(this.$scope.$workspaceScope)) {
                    $state.go($state.current, $stateParams, { reload: true, inherit: false, notify: true }).then( function () {
                        self.rootActions.goOutEditionMode();
                    });
                } else {
                    $state.go('^', {}, { reload: true, inherit: false, notify: true }).then( function () {
        ...
        MFRootScopeActions.prototype.canAddToList = function () {
            ...
            var canAddToListResult = false;
            var isWorkspace = !angular.isUndefinedOrNull(this.$scope.$workspaceScope);

            for(var i=0;i<this.$scope.$listScopes.length;i++){
                var currListScope = this.$scope.$listScopes[i];

                if( currListScope.viewConfig.canAdd && (
                        isWorkspace && !angular.isUndefinedOrNullOrEmpty(currListScope.viewConfig.groupDetailState) ||
                        !isWorkspace && !angular.isUndefinedOrNullOrEmpty(currListScope.viewConfig.detailState)
                    )
                ){
                    canAddToListResult = true;
                    break;
                }
            }

            return canAddToListResult;
        };
        ...
        MFRootScopeActions.prototype.navigateNew = function () {
            ...
            for (var listScopeKey = 0; listScopeKey < this.$scope.$listScopes.length; ++listScopeKey) {
                if (!angular.isUndefinedOrNull(this.$scope.$listScopes[listScopeKey].actions.selectedScopeItem)) {
                    this.$scope.$listScopes[listScopeKey].actions.selectedScopeItem.isSelectedItem = false;
                    this.$scope.$listScopes[listScopeKey].actions.selectedScopeItem = null;
                }
            }
