    ...
    function(MFDirectivesHelper, $timeout,MFActionOnScroll) {
        ...
        return {
            ...
            controller: function controller($scope, $attrs, $element) {
                ...
                if (angular.isUndefinedOrNullOrEmpty($attrs.mfItemsDisplayStep)) {
                    //Default number of additional items to display when scrolling down
                    $scope.mfItemsDisplayStep = 3;
                } else {
                    $scope.mfItemsDisplayStep = $attrs.mfItemsDisplayStep;
                }

                if (angular.isUndefinedOrNullOrEmpty($attrs.mfRowHeight)) {
                    $scope.mfRowHeight = 45;
                } else {
                ...
                $scope.$parent.$watch($attrs.mfField,function(newValue,oldValue){
                    // number of items displayed at the beginning, when list loaded
                    $scope.itemsDisplayedInList = Math.round(MFActionOnScroll.htmlElement.clientHeight/$scope.mfRowHeight)+$scope.mfItemsDisplayStep;
                    //approximation because MFActionOnScroll.htmlElement.clientHeight is greater than the true height of the list, because of the control bar.
                    ...
                    if (!angular.isUndefinedOrNullOrEmpty($scope.mfField)) {
                       var vmList = MFDirectivesHelper.getFieldViewModel($scope.mfField,$scope, false);
                        if ($scope.itemsDisplayedInList < vmList.length) {
                            console.debug('loading '+$scope.mfItemsDisplayStep+' items more...');
