...
angular.module('mfui').factory('MFListHelper', ['MFCordova', '$window', 'MFSyncPromiseProvider', function (MFCordova, $window, $qSync) {
     ...
     *
     * @description
     * This function initializes self.isMobileDevice
    ...
    MFListHelper.prototype.getTemplate = function getTemplate(tElement, mfSublist) {
        ...
        if (self.isMobileDevice) {
            ...
            tElement.html('');
            if (mfSublist) {
                parentTemplate = angular.element('<div class="mflist-sublist">' +
                    '</div>');
                template = angular.element(
                    '<div ng-repeat="subitem in item.list"' +
                ...
                if (self.hasMfListChildren) {
                    ...
                    parentTemplate = angular.element('<accordion class="mdk-mflist-2D" close-others="true">' +
                        '</div>');
                    template = angular.element(
    ...
    /* MFListHelper.prototype.hasMfListChildren = function () {
     var self = this, lbReturn = false;
     if (!angular.isUndefinedOrNullOrEmpty(self.childrenMfList)) {
     lbReturn = true;
     }
     return lbReturn;
    ...
    MFListHelper.prototype.initView = function (tElement) {
        ...
        for (; i < length; i++) {
            ...
            } else if (dom[i].className === 'mdk-floating-button') {
