...
angular.module('mfui').factory('MFDirectivesHelper',['MFIntegerConverter', function(MFIntegerConverter) {
    ...
    MFDirectivesHelper.prototype.checkOptionalBooleanAttributes = function checkOptionalBooleanAttributes(attrsObj, attrNames, defaultValueIfNotDefined) {
        var optionnalBooleanValue = defaultValueIfNotDefined;
        if(angular.isUndefinedOrNull(defaultValueIfNotDefined)){
            defaultValueIfNotDefined = false;
            optionnalBooleanValue = defaultValueIfNotDefined;
        }
        ...
        for (var i = 0; i < attrNames.length; i++) {
            if (angular.isUndefined(attrsObj[attrNames[i]])) {
                ...
                optionnalBooleanValue = true;
            }
            else {
                optionnalBooleanValue =  attrsObj[attrNames[i]] = (attrsObj[attrNames[i]] === 'true');
            }
        }
        return optionnalBooleanValue;
    };
    ...
    MFDirectivesHelper.prototype.watchOnChange = function watchOnChange(attrs, scope) {
        if (!angular.isUndefined(attrs.mfOnchange)) {
            ...
            scope.$parent.$watch(attrs.mfField, function(newValue, oldValue) {
                if (!firstTime) {
                    scope.$parent.viewConfig.viewModelFactory.mfOnchange(newValue, scope.$parent);
                } else {
                    firstTime = false;
                }
            }, true);
...
/**
    ...
    MFDirectivesHelper.prototype.chooseTemplate = function chooseTemplate(attrs,name) {

        if(!angular.isUndefinedOrNull(name)){
            var templatePath = 'mfui/directives/' + name + '/'+ name;
            if(checkOptionalBooleanAttributes(attrs, ['mfHideLabel'], false)){
                templatePath = templatePath + 'NoLabel';
            }
