...
angular.module('mfui').directive('mfFixedList', ['MFDirectivesHelper', '$modal', '$injector', 'MFFormModals',

    function(MFDirectivesHelper, $modal, $injector, MFFormModals) {
...
//            },
            controller: function controller($scope, $attrs, $element) {
                ...
                if (angular.isUndefined($attrs.mfDetailTemplate)) {
                    ...
                    template = $attrs.mfDetailTemplate;
                }
...
//                            $scope.mfField.push(selectedItem);
                        }
                        else {
                            // item is modified, update the list
//                            $scope.mfField.splice( indexSelected, 1, selectedItem );
...
//                        fixedListScope.mfField.splice(index, 1);
            ...
            compile: function compile(tElement, tAttrs) {
                ...
                var deleteItemTemplate = angular.element(
                    '<div class="col-sm-10 pull-right"><span class="glyphicon glyphicon-trash" ng-click="deleteItem($index)" ng-readonly="mfReadonly" ng-hide="!actions.isInEditionMode()" style="z-index:9999;"></span></div>');

                var openItemTemplate = angular.element('<div ng-click="open(item)"></div>');

                var template = angular.element('<div class="panel panel-default">' +
                '<div class="panel-heading">' +
                '    <ul class="list-inline">' +
                '         <li>' +
                '              <span class="glyphicon glyphicon-plus margin-top-8 green" ng-click="open()" ng-readonly="mfReadonly" ng-hide="!actions.isInEditionMode()"></span>' +
                '           </li>' +
                '           <li>' +
//                '               <span>{{headTitle}} ({{mfField.length}})</span>' +
               '               <span>{{headTitle}} ({{items.length}})</span>' +
                '           </li>' +
                '       </ul>' +
                '   </div>' +
                '   <div id="fixed-list-body" class="panel-body">' +
//              '       <div ng-repeat="item in mfField track by $index" class="well">' +
               '       <div ng-repeat="item in items track by $index" class="well">' +
                '       </div>' +
                '   </div>' +
                '</div>');
